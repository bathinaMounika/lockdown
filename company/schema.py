...
class EmployeeNode(DjangoObjectType):
    class Meta:
        model = Employee
        filter_fields = ['employee_name', 'employee_city__city_name', 'employee_title__title_name']
        interfaces = (graphene.relay.Node,)
...
class UpdateEmployee(graphene.relay.ClientIDMutation):
    ...
    class Input:
        ...
        def mutate_and_get_payload(root, info, **input):
            ...
            employee.employee_city = input.get('employee_city')
            employee.employee_title = input.get('employee_title')
